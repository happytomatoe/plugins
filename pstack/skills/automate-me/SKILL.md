...
This skill orchestrates three others: an inline mining pass (see step 1), the local skill-authoring workflow, and the **unslop** skill (prose discipline). It sequences them; it doesn't replace them.

## Flow

### 0. Check for an existing skill

Look in the project's established Pi skill directory and the user-level Pi skill directory for a matching `*-mode/SKILL.md`. If one exists, confirm intent with a normal chat question (unless they already said "update my skill" or similar):
...
Use Pi session tools to locate recent conversations in the active workspace. Do not scan other workspaces.
...
Mining misses intent that hasn't come up yet. Ask a normal chat question rather than asking the user to type from scratch. Lower cognitive load, higher hit rate.
...
Use the local skill-authoring workflow to author the skill. Placement:

- Path: preserve an existing mode skill's category. For a new mode, use the project-local Pi skill directory/<handle>/<handle>-mode/SKILL.md when the repo has an established personal category for that handle; otherwise default to the project-local Pi skill directory/<handle>-mode/SKILL.md in the project (or the user-level Pi skill directory/<handle>-mode/ if the user prefers a personal skill).
...
- The local skill-authoring workflow: skill authoring process and writing guidelines.
